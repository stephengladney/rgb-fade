...
function gradient(color1, color2, length) {
    ...
    const redInterval = redStep !== 0 ? Math.floor((length - 1) / redRemainder) : 0;
    const greenInterval = greenStep !== 0 ? Math.floor((length - 1) / greenRemainder) : 0;
    const blueInterval = blueStep !== 0 ? Math.floor((length - 1) / blueRemainder) : 0;
